...
if(!intBaiduCproCn) {var intBaiduCproCn=1;} else {intBaiduCproCn++;}
...
function fv()
{
	...
	{
		var fl = navigator.plugins["Shockwave Flash"];
		if (fl && fl.description)
		{
			ver = fl.description.replace(/([a-zA-Z]|\s)+/, "").split(".")[0];
		}
	}
	else if (navigator.userAgent.toLowerCase().indexOf("ie"))
	{
		try
		{
			var fl = new ActiveXObject("ShockwaveFlash.ShockwaveFlash.7");
		}
		catch (e)
		{
			try
			{
				var fl = new ActiveXObject("ShockwaveFlash.ShockwaveFlash.6");
				ver = 6;
		...
		if (fl != null)
		{
			ver = fl.GetVariable("$version").split(" ")[1].split(",")[0];
		}
	}
	return ver;
}
...
function baiduCproGetUri(strBaiduCproRefer)
{
	...
	else
	{
		...
		else if(h<arrBaiduCproConfig['h']||w<arrBaiduCproConfig['w'])
		{
			cif+=8;
		}
		strBaiduCproUiUri+='&if='+cif;
	}
	...
	strBaiduCproUiUri += '&fv='+fv();	
	strBaiduCproUiUri += '&cn=' + intBaiduCproCn;
	if (strBaiduCproRefer != '')
	{
		strBaiduCproUiUri += '&refer=' + strBaiduCproRefer;
	}
...
{
	var strBaiduCproRefer = typeof(encodeURIComponent)=='function' ? encodeURIComponent(document.referrer) : escape(document.referrer)
	return '<iframe marginWidth=0 marginHeight=0 scrolling=no frameBorder=0 width='+arrBaiduCproConfig['w']+' height='+arrBaiduCproConfig['h']+' allowtransparency="1"  ;  src="'+baiduCproGetUri(strBaiduCproRefer)+'"></iframe>';	
}
function baiduCproIFrame()
{
	if(intBaiduCproCn>3) return "";
